...
const voiceCallConfigSchema = {
  ...
  uiHints: {
    ...
    "streaming.realtimeVoice": { label: "Realtime Conversation Voice", advanced: true },
    "tts.provider": {
...
const VoiceCallToolSchema = Type.Union([
  Type.Object({
    ...
    message: Type.Optional(Type.String({ description: "Intro message (used for notify mode or context)" })),
    mode: Type.Optional(Type.Union([Type.Literal("notify"), Type.Literal("conversation")])),
    instructions: Type.Optional(
      Type.String({
        description:
          "Full instructions for autonomous AI conversation via OpenAI Realtime. When provided, the AI conducts the call independently and returns a transcript. Include: language, persona, goals, conversation style. Do NOT include voice selection — it is configured separately.",
      }),
    ),
  }),
  Type.Object({
            ...
            switch (params.action) {
              case "initiate_call": {
                ...
                if (!instructions && !message) {
                  throw new Error("message or instructions required");
                }
                const to =
                  typeof params.to === "string" && params.to.trim()
                    ? params.to.trim()
                ...
                const result = await rt.manager.initiateCall(to, undefined, {
                  message: instructions ? undefined : message,
                ...
                // If instructions provided, wait for the call to complete
                // and return the full conversation transcript
                if (instructions) {
                  const timeoutMs = rt.config.maxDurationSeconds * 1000 + 30000;
                  try {
                    const transcript = await rt.manager.waitForCallEnd(
                      result.callId,
                      timeoutMs,
                    );
                    ...
                    return json({
                      ...
                      duration: call?.endedAt && call?.answeredAt

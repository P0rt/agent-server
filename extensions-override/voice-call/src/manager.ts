...
export class CallManager {
  ...
  private callCompleteWaiters = new Map<
    CallId,
    {
  ...
  private getContext(): CallManagerContext {
    return {
      ...
      callCompleteWaiters: this.callCompleteWaiters,
      onCallAnswered: (call) => {
  ...
  /**
   * Wait for a call to complete (used by realtime conversation mode).
   * Resolves with the conversation transcript when the call ends.
   */
  waitForCallEnd(callId: CallId, timeoutMs: number): Promise<ConversationTranscriptEntry[]> {
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        this.callCompleteWaiters.delete(callId);
        reject(new Error("Call completion timeout"));
      }, timeoutMs);

      this.callCompleteWaiters.set(callId, { resolve, reject, timeout });
    });
  }

  /**
   * Resolve a pending call-complete waiter with the conversation transcript.
   */
  resolveCallComplete(callId: CallId, transcript: ConversationTranscriptEntry[]): void {
    const waiter = this.callCompleteWaiters.get(callId);
    if (waiter) {

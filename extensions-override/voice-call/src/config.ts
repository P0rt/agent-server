...
export const VoiceCallStreamingConfigSchema = z
  .object({
    ...
    /** OpenAI Realtime model for conversation mode */
    realtimeModel: z.string().min(1).default("gpt-realtime-mini"),
    /** Default voice for Realtime conversation */
    realtimeVoice: z.string().min(1).default("coral"),
  })
  .strict()
  .default({
    enabled: false,

...
type CallCompleteWaiterContext = Pick<CallManagerContext, "callCompleteWaiters">;

export function resolveCallCompleteWaiter(
  ctx: CallCompleteWaiterContext,
  callId: CallId,
  transcript: ConversationTranscriptEntry[],
): void {
  const waiter = ctx.callCompleteWaiters.get(callId);
  if (!waiter) return;
  clearTimeout(waiter.timeout);
  ctx.callCompleteWaiters.delete(callId);
  waiter.resolve(transcript);
}

export function rejectCallCompleteWaiter(
  ctx: CallCompleteWaiterContext,
  callId: CallId,
  reason: string,
): void {

...
export async function initiateCall(
  ...
  const initialMessage = opts.instructions ? undefined : opts.message;
  const mode = opts.mode ?? (opts.instructions ? "conversation" : ctx.config.outbound.defaultMode);
...
export async function endCall(
  ...
  try {
    ...
    // arrives via onCallComplete. The timeout on waitForCallEnd handles leaks.

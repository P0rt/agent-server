...
import {
  ...
  rejectCallCompleteWaiter,
  rejectTranscriptWaiter,
  resolveCallCompleteWaiter,
...
export function processEvent(ctx: EventContext, event: NormalizedEvent): void {
  ...
  if (ctx.processedEventIds.size > MAX_PROCESSED_EVENT_IDS) {
    const it = ctx.processedEventIds.values();
    for (let i = 0; i < MAX_PROCESSED_EVENT_IDS / 2; i++) {
      ctx.processedEventIds.delete(it.next().value!);
    }
  }
  ...
  switch (event.type) {
    ...
    case "call.ended":
      ...
      // NOTE: do NOT resolve callCompleteWaiters here — the real transcript
      // arrives via onCallComplete (media stream handleStop). Resolving here
      // with [] would race and return an empty transcript.
    ...
    case "call.error":
      if (!event.retryable) {
        ...
        rejectCallCompleteWaiter(ctx, call.callId, `Call error: ${event.error}`);
        ctx.activeCalls.delete(call.callId);
        if (call.providerCallId) {
          ctx.providerCallIdMap.delete(call.providerCallId);
        }

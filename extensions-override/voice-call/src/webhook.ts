...
import { RealtimeConversationProvider } from "./providers/realtime-conversation.js";
...
export class VoiceCallWebhookServer {
  ...
  /**
   * Initialize media streaming with OpenAI Realtime STT and Conversation.
   */
  private initializeMediaStreaming(): void {
    ...
    // Create conversation provider for speech-to-speech mode
    const conversationProvider = new RealtimeConversationProvider({
      apiKey,
    ...
    // Cache providerCallId → internal callId so onCallComplete works
    // even after call.ended deletes the call from activeCalls.
    const providerToInternal = new Map<string, string>();

    const streamConfig: MediaStreamConfig = {
      sttProvider,
      conversationProvider,
      getRealtimeInstructions: (providerCallId) => {
        const call = this.manager.getCallByProviderCallId(providerCallId);
        if (!call?.metadata?.realtimeInstructions) {
          return null;
        }
        const rt = call.metadata.realtimeInstructions as {
          instructions: string;
          voice?: string;
        };
        return rt;
      ...
      onTranscript: (providerCallId, transcript) => {
        ...
        // Auto-respond — but NOT for conversation-mode calls (OpenAI handles those)
        if (call.metadata?.realtimeInstructions) {
          return;
        }
      ...
      onConnect: (callId, streamSid) => {
        ...
        // Cache the providerCallId → internal callId mapping for onCallComplete
        const connectedCall = this.manager.getCallByProviderCallId(callId);
        if (connectedCall) {
          providerToInternal.set(callId, connectedCall.callId);
        }

        // For conversation-mode calls, OpenAI Realtime handles the greeting.
        // For STT-mode calls, speak the initial message via TTS.
        if (connectedCall?.metadata?.realtimeInstructions) {
          console.log(`[voice-call] Conversation mode — OpenAI handles greeting for ${callId}`);
          return;
        }
      ...
      onCallComplete: (providerCallId, transcript) => {
        console.log(
          `[voice-call] Conversation complete for ${providerCallId}: ${transcript.length} entries`,
        );

        // Use cached mapping — call record may already be deleted by call.ended event
        const internalCallId = providerToInternal.get(providerCallId);
        const call = this.manager.getCallByProviderCallId(providerCallId);
        if (call) {
          call.metadata = {
            ...(call.metadata ?? {}),
        ...
        // Resolve waiter using cached internal callId (works even after call deletion)
        const resolveId = call?.callId ?? internalCallId;
        if (resolveId) {
          this.manager.resolveCallComplete(resolveId, transcript);
        }

        // Cleanup cache
        providerToInternal.delete(providerCallId);
      },
      onDisconnect: (callId) => {
        ...
        // Clean up providerCallId→internalCallId cache
        providerToInternal.delete(callId);
        if (this.provider.name === "twilio") {
    ...
    console.log("[voice-call] Media streaming initialized (with conversation support)");
  }
